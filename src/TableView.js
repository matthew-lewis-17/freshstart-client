...
if (isLoading){
  ...
  <svg role="status" className="inline w-16 h-16 mr-2 text-gray-200 animate-spin dark:text-gray-600 fill-soft-green" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
...
return (
  <div>
    <div className='flex'>
      ...
      <div className="bg-soft-grey absolute inset-0 p-2 flex flex-col justify-center space-y-4">
        <div className="text-3xl text-black font-extrabold text-center">{getHeader(accessVar)}</div>
        <SliderColumnFilter thisData={accessVar} setAccessVar={setAccessVar}/>
        <div className="text-3xl text-black font-extrabold text-center">{curRowLength} counties remaining...</div>
    </div>
      </Modal>
      </div>
      </div>
